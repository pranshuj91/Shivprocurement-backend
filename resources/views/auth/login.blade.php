    ...
    <div class="w-full max-w-md bg-white border border-zinc-200/80 rounded-2xl p-8 shadow-xl shadow-zinc-200/50 relative overflow-hidden">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5 relative">
            ...
            <button type="submit" 
                ...
                <i data-lucide="log-in" class="w-4 h-4"></i> Access Dashboard
            </button>
        </form>

        <div class="mt-8 border-t border-zinc-100 pt-5 text-center">
            <p class="text-[11px] text-zinc-500">
                New procurement manager? 
                <a href="{{ route('signup') }}" class="font-semibold text-[#0d2818] hover:text-[#163a23] transition underline decoration-dotted">Create an account</a>
            </p>
        </div>
    </div>
